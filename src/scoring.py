...
def has_scoring_data(results: Results) -> bool:
    """True if any official results exist (partial or complete)."""
    return bool(
        results.champion
        or results.runner_up
        or any(results.semi_finalists)
        or any(results.group_winners.values())
        or any(results.bonuses.values())
    )


def results_are_set(results: Results) -> bool:
    """True if the tournament winner and runner-up are known."""
